...
ml parallel
klen=(8 10 15 20 50 100)
window=(8 10 20 50 100 200 500)
precision=($(seq 23 1 26))
...
for p in "${precision[@]}"; do
  ...
  parallel --will-cite --jobs 28  --halt now,fail=1 --eta \
    ...
    hammock $fastafiles $fastafiles --mode D --kmer_size {1} --window_size {2} --precision $p" --minimizer\
  ::: "${klen[@]}" ::: "${window[@]}"
done
