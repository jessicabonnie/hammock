...
```{r dataprep}
cobindo <- fread("./results/cobind_CTCF.txt",fill=TRUE,sep=" ") %>%
...
sketch <- fread("./results/10000_p10000_jaccA.csv") %>% rename(A.name=bed1, B.name=bed2, sketchA=jaccA)
sketch$B.name <- gsub(".bed", "", gsub("_hg38","", x=gsub("_c3","",x=sketch$B.name)))
sketch$A.name <- gsub(".bed", "", gsub("_hg38","", x=gsub("_c3","",x=sketch$A.name)))

tfs <- fread("./results/remap_encode_jaccC.csv") %>% rename(A.name=bed1, B.name=bed2)
