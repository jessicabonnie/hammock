...
class HyperLogLog(AbstractSketch):
    ...
    def _register_counts(self) -> np.ndarray:
        ...
        # Make sure the bincount has enough bins for the hash size
        # Add +2 to account for the max+1 possible value in some calculations
        # Make sure the bincount has enough bins for the hash size
        # Add +2 to account for the max+1 possible value in some calculations
        return np.bincount(self.registers, minlength=self.hash_size - self.precision + 2)
    ...
    def estimate_cardinality(self, method: str = 'ertl_improved') -> float:
        ...
        if method == 'original':
            ...
            # Fall back to original method as it's more stable
            return self.original_estimate()
        else:
    ...
    def ertl_improved_estimate(self) -> float:
        ...
        # Check if registers have valid indexes for counts
        # Make sure we're not accessing beyond the array bounds
        if len(self.register_counts) <= max_register_value + 1:
            # If we don't have enough elements, fall back to original estimate
            return self.original_estimate()

        # Get counts of maxed registers
        n_maxed_registers = self.register_counts[max_register_value + 1]
        ...
        for i in range(max_register_value, 0, -1):
            if i < len(self.register_counts):  # Make sure we don't access beyond array bounds
