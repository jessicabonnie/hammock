...
`docs/data/sweep_precision_maurano_p18_t16.csv` (20 Maurano DHS files, 190
unique off-diagonal pairs, subB=1.0, p ∈ {12,14,16,18,20,22,24},
3 replicates, medians). The t=8 companion (`sweep_precision_maurano_p18_t8.csv`)
...
- **Two gates the script enforces and dies on.** Pair count must be exactly 190
  ($\binom{20}{2}$ unique off-diagonal pairs) — at 400 every MAE would be ~5%
  low from twenty exact-1.0 rows and would also double-count reciprocal pairs;
  accuracy at t=8 must be *bit-identical* to t=16, since
