...
**Superseded a few hours later by the "Second correction" below — this
paragraph's own reasoning did not hold up.** `estimator_compare.py`'s
"Figure:" line and the `docs/scripts/mode_d_*.R` files' "structured
figure-index table" both turned out to be `docs/paper_outline.md`-only
citations after all, the same class this paragraph explicitly claims they
aren't. Left here, uncorrected in place, as the record of the mistake
itself — see below for what actually held up on a second, more skeptical
pass.

Step 2's file count was **15**, not 22, after this first correction.

**Second correction, 2026-08-12, same day — the first correction applied
its own stated standard inconsistently.** The user asked for a fresh round
of adversarial review specifically to check whether the first revert went
far enough. Two independently launched reviewers (a revert-correctness
lens and a dedicated scope-re-audit lens) each checked every one of the 15
"kept" files directly against `paper/outline.md`/`paper/draft.md` — not
`docs/paper_outline.md` — and converged on the same result: **7 more of the
15 kept files rest on exactly the citation class the first correction had
just called insufficient.**

- `paper/estimator_crossover/plot_estimator_crossover.R` and its sole
  feeder `experiments/bedtools_benchmark/estimator_compare.py` — zero
  mentions of "estimator_crossover" anywhere in `paper/outline.md` or
  `paper/draft.md` (grepped both directly). The only citation is
  `docs/paper_outline.md:320`'s "Figure:" line — the exact citation class
  the first correction ruled insufficient for the 7 files it reverted. The
  first correction's own stated justification for keeping this pair ("its
  own 'Figure:' line in `docs/paper_outline.md`'s Sec 6.1") was, on
  inspection, a `docs/paper_outline.md`-only citation described in words
  that made it sound stronger than it was.
- `docs/scripts/mode_d_{violins,lines,metric_tradeoff,bedtools_vs_modeB_scatter}.R`
  — zero mentions of any of these scripts or their output figures anywhere
  in `paper/outline.md`/`paper/draft.md`. They write to `docs/figures/`,
  not `paper/figures/` — not even in the manuscript's figure directory.
  Their only citation is `docs/paper_outline.md`'s figure-index table,
  under *that* document's own separate numbering scheme, independently
  reconfirmed by both reviewers to disagree with the live manuscript for
  the same figure slots (`docs/paper_outline.md`'s "Figure 3" is
  `mode_d_bedtools_vs_modeB_scatter.png`; the live paper's actual Figure 3
  is `pairwise_scaling.png` — a different image entirely). The first
  correction's stated distinction ("a direct, structured citation, not a
  loose two-hop prose chain") didn't survive scrutiny: a structured table
  row in a document already found superseded relative to the live
  manuscript is still not a live-manuscript citation, structured or not.
- `paper/parameter_objective_tradeoff/plot_parameter_objective_tradeoff.R`
  (the non-`_estimators` sibling) — named in the live paper's prose
  (`paper/draft.md:122`/`paper/outline.md:95`: *"the existing
  `plot_parameter_objective_tradeoff.R` is retained unchanged as the
  single-estimator diagnostic workflow"*), but explicitly **not** as the
  generator of any current figure — Figure 7 is generated by the
  `_estimators.R` sibling, correctly kept. This file's own output is never
  cited. Being named in prose is not the same as feeding a figure or stat
  the manuscript reports.

**Reverted, commit `f37bec6`** — all 7 confirmed byte-identical to `6cce6d3`
(the commit before any Step 2 work) via direct `diff`, not just re-asserted.

**Step 2's file count is now 8, not 15, not 22.** All 8 independently
re-verified — by both subagent reviewers and then a third time, directly,
by the implementing session itself against `paper/outline.md`/`paper/draft.md`'s
actual text, quoting the exact "Figure N generation." paragraph for each —
to have a genuine direct citation in the live manuscript:
`paper/interval_accuracy/plot_interval_accuracy.R` (Figure 4,
`paper/draft.md:92`), `paper/sequence_tissue_clustering/plot_sequence_tissue_clustering.R`
(Figure 6 + Fig S5a, `paper/draft.md:112,271`),
`paper/cross_reference_identity/plot_cross_reference_identity.R` (Figure 5
+ Fig S5b, `paper/draft.md:101,272`),
`paper/parameter_objective_tradeoff/plot_parameter_objective_tradeoff_estimators.R`
(Figure 7, `paper/draft.md:122`), `experiments/maurano_dhs_validation/analyze.R`
(writes `docs/data/mode_d_summary.csv`, the exact file Figure 7's
generation note names as its data source),
`experiments/bedtools_benchmark/sweep.py` (Figure S8 +
Figure 3B cross-check, `paper/draft.md:306`),
`experiments/ref-comparison/estimator_ie_crossref.py` (Table S5,
`paper/draft.md:273` — output-schema match confirmed, no explicit driver
call site located, a residual asterisk, not blocking), and
`experiments/subB_mixed_stride/run_sweep.py` (Figure S9,
`paper/draft.md:282`).

**Lesson, now doubled:** a `docs/paper_outline.md` citation is not a
sufficient basis for "in scope" at all — not even when it's a structured
table row rather than prose, and not even when a file is one hop removed
from a file that has one. The only bar that held up on inspection both
times was a direct citation in `paper/outline.md`/`paper/draft.md` naming
the script or its output file. Applying a corrected standard to the files
that prompted the correction, while not re-checking the rest of what
"passed" under the old standard, is exactly how the first correction
still left 7 files wrong. If this rename ever needs another scope check,
re-verify **every** kept file's citation directly, every time — do not
assume a file that "passed" under an earlier round still passes under a
newly-tightened one.
