...
RSS, and shutdown/error-handling complexity. The branch propagates
`sequence_hll_hash` through each worker task and reconstructed sketch; this is
a required invariant because serial and multi-worker runs must use the same
Mode D feature-hash contract.
