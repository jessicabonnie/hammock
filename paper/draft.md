...
- **Real-data timing result (Figure 3B):** ~~on the 20 Maurano fetal-tissue DNase hypersensitivity BED files, hammock is faster than the parallelized BEDTools workflow even without subsampling.~~ **Superseded 2026-08-09.** That claim rested on a BEDTools baseline that was process-creation bound and whose achieved parallelism was never recorded. Re-measured at p=18 against the corrected one-process-per-pair harness (job 29651773, `docs/precision-frontier-maurano.md`), hammock is at **parity or slightly behind** on this corpus: 1.02× at 16 threads and 0.92× at 8. That is the expected result — N=20 is far below the N≈64 crossover, and Panel A measures 14.75× at N=256 on the same code. Note the caveat cuts against us in both directions: the BEDTools leg here still ran at only ~1.4× parallel efficiency, so a properly parallel baseline would make these numbers *worse*, not better. Do not restate the old sentence.
...
**NOTE: this figure is likely to change.** The BEDTools baseline behind it has been superseded. A pairwise `bedtools jaccard` workflow launches one process per pair and is process-creation bound on these nodes (~123 exec/s, not scaling with cores), so its achieved parallel efficiency was neither controlled nor recorded and varies 1.17×–2.86× between nodes. BEDTools also *regresses* past t=8 (3.30× at t=8 → 1.89× at t=48), while hammock is monotonic to 18.81×. The harness has since been changed to run one process per pair instead of three (worth ~2.1× to the baseline) and to record `mean_bedtools_parallel_eff` per row. Panel A is being re-measured at p=18 against the corrected baseline (jobs 29652408 and 29652432, launched 2026-08-09). Expect the panel structure, the quoted speedups, and the Panel B framing to move. Evidence: `docs/bedtools-parallelism-caveat.md`, `docs/precision-frontier-maurano.md`.
