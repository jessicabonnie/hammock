...
![Figure 6](../experiments/maurano_dhs_validation/figures/mode_d_best_dendrogram.png)

**Figure 6. Sequence-mode sketches recover fetal-tissue organization in the Maurano DNase hypersensitivity collection.** Twenty fetal-tissue DNase-seq interval sets spanning ten annotated tissue labels were converted to interval-derived sequence and compared with the ARI-optimal sequence-mode setting, using the minimizer-only `jaccard_similarity` score at \(k=10\), \(w=30\), and the selected sweep precision. Average-linkage hierarchical clustering was performed on \(1-J\). Leaf labels are colored by tissue, while blue boxes mark the ten clusters obtained by cutting the dendrogram at the number of annotated tissue classes. The resulting partition achieved an adjusted Rand index of \(0.910\) and normalized mutual information of \(0.961\). Most tissues form distinct groups; the primary discrepancies are the separation of the two brain samples and the grouping of one back-muscle sample with the leg-muscle samples. The figure therefore shows that sequence-derived similarity recovers most of the known tissue organization while retaining finer distinctions among closely related muscle samples.

**Figure 6 generation.** The figure is generated by `experiments/maurano_dhs_validation/analyze.R` and written to `experiments/maurano_dhs_validation/figures/mode_d_best_dendrogram.png`. The analysis selects the configuration with the highest adjusted Rand index from the minimizer-only `jaccard_similarity` results and draws the predicted-cluster boxes directly on the dendrogram.
