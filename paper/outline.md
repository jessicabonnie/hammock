...
**Figure 5. Sequence sketches group samples by tissue rather than by reference genome.** Three ENCODE H3K27ac ChIP-seq samples (heart ENCSR175ABH, liver ENCSR864OOO, lung ENCSR954JMZ) were independently aligned and peak-called against GRCh37, GRCh38, and CHM13, giving nine peak sets. Each was converted to sequence against its own reference and sketched in sequence mode (\(k=10\), \(w=10\), \(p=24\); broad peaks). (A) UPGMA clustering on \(1 - J\) recovers the three tissues as clean clades, with the three references for a given tissue nesting inside that tissue's clade in every case. Reference choice therefore acts as a within-tissue perturbation rather than a between-group split, which is the property that makes cross-reference comparison meaningful. CHM13 is consistently the most distant reference within each clade, consistent with its divergence from the GRCh assemblies. (B) Distribution of pairwise similarity for the three comparison classes over the 36 unique file pairs: same tissue with different references (\(n=9\), \(J = 0.988-0.998\), median \(0.991\)), different tissue with the same reference (\(n=9\), \(J = 0.910-0.935\), median \(0.920\)), and different tissue with different references (\(n=18\), \(J = 0.906-0.934\), median \(0.919\)). Every same-tissue pair ranks above every different-tissue pair, whichever references are involved. The balanced 9-versus-9 contrast — changing the reference while holding tissue fixed, against changing the tissue while holding the reference fixed — isolates the claim that tissue dominates reference: the two classes are completely separated (AUC \(= 1.00\)) with a median difference of \(0.070\). Significance is assessed by an exact constrained label permutation rather than a rank-sum test: because the nine peak sets form a \(3 \times 3\) tissue-by-reference design, the 36 pairs are not independent observations, so we instead permute the tissue labels within each reference, which preserves the reference composition, and recompute the AUC. This gives \(p = 0.028\) over the \((3!)^3 = 216\) constrained labelings. Since a global renaming of the three tissues leaves every comparison class unchanged, those 216 labelings induce only \(216/3! = 36\) distinct classifications and \(1/36 \approx 0.028\) is the smallest attainable p-value; the observed AUC of \(1.00\) is the strict maximum over the null, whose largest value is \(0.852\). The separation is a margin rather than a gap in absolute terms — all nine sketches share substantial repeat-driven k-mer content, so even different-tissue similarity is high — but the ordering is unambiguous and the clustering in (A) follows from it.

**Figure 5 generation.** The combined figure is generated by `paper/cross_reference_identity/plot_cross_reference_identity.R` and written to `paper/figures/cross_reference_identity.png`. Inputs are `docs/data/exp_a_broad_k10_w10.csv` (staged from `experiments/ref-comparison/results/exp_a/broad/k10_w10/exp_a_mnmzr_p24_jaccD_k10_w10.csv`) and `docs/data/exp_a_metadata.tsv` (staged from that experiment's `config/`); `docs/data/exp_a_narrow_k10_w10.csv` is staged alongside for the narrow-peak replicate. The script differs from the experiment's own plotting scripts in three respects: mirrored pairs are collapsed so each unordered pair is counted once, the reported similarity is the minimizer-only Jaccard rather than the minimizer-plus-ends variant, and panel B's p-value comes from an exhaustive enumeration of the 216 within-reference tissue-label permutations (computed in the script, not read from a cached file) rather than from a rank-sum test over the pairs.
