...
> **Update, 2026-08-10 00:30:** Figure 3 (both panels) has been re-measured on the fully-fixed harness and its numbers in this file are now current — **9.21× at N=512** (corrected from an intermediate 10.17× after a small-N noise bug in that rerun was found and fixed; see below), 1.16× slower/1.58×/2.49× on Maurano. Supplementary Figs S6, S7, and S8 have also been re-measured and are current (`docs/paper_outline.md` carries their numbers). See `docs/bedtools-baseline-retraction.md` for the ledger. **Superseded later the same day** by the figure-simplification update below: those three Maurano numbers were the register-equality (`--no-metrics`) wall times; the current Panel B plots the `jaccard_similarity_ie` arm instead, which costs more (full metrics block) and reads **1.33× slower/1.41× faster/2.04× faster**.
...
**Panel A and B re-measured on the corrected harness, 2026-08-09/10 (job 29671317, node c529, single allocation; run_bedtools_maurano rerun) — this is the current, checked number.** Four harness defects were found and fixed over 2026-08-09, all of which had inflated the BEDTools baseline (full history: `docs/bedtools-baseline-retraction.md`). The last and largest: the BEDTools environment module exports `LD_LIBRARY_PATH` with 17 directories of which BEDTools uses 4, and the dynamic linker searched the other unused ones — on a network filesystem — on every one of N² process launches. Fixing it moved the N=512 speedup from a previously-reported 27.61× to an intermediate 10.17×. That intermediate number itself then turned out to have a second defect: its N=2/N=4 cells were noise-corrupted at n=3 (one bad bedtools replicate made the table read an impossible cost *drop* from N=2 to N=4). A follow-up rerun, N≤32 at 20 replicates and N≥64 at 3, all within one allocation to avoid a cross-node confound, produced the current, clean table: **N=512 speedup 9.21×**, and bedtools reliably wins below N≈64 (monotonic, tight replicate spread — e.g. N=32 spans 2.52–2.63 s across 20 reps) rather than the earlier draw's noise-dominated small-N cells. Verified: hammock's own wall time at N=512 moved under 1% across the whole chain of reruns (71.65 s → 70.32 s → 70.97 s), confirming only the BEDTools arm's correction is doing the work.

BEDTools' achieved parallelism on this corpus is now 0.4–5.1 of 16 cores across N=2–512 (`mean_bedtools_parallel_eff`) — still far from saturated, so the equal-core-budget framing should be stated explicitly. Panel B (Maurano, 20 files, subB sweep) was re-measured on the same fixed call path: the BEDTools reference is **7.07 s** (was 7.34 s under the partial fix, 11.08 s originally), and the hammock bars read **1.16× slower / 1.58× faster / 2.49× faster** at subB = 1.0 / 0.1 / 0.01. hammock does not beat BEDTools without subsampling on this 20-file corpus — consistent with N=20 sitting well below the crossover visible in Panel A (between N=32 and N=64). The panel B label is generated from the sign, not hardcoded to "faster".

**These are the register-equality (`--no-metrics`) numbers; Panel B no longer plots them.** As of the 2026-08-10 figure simplification above, Panel B reads hammock's `jaccard_similarity_ie` arm (full metrics block, costlier than `--no-metrics`) at each subB level: **9.4 s / 5.0 s / 3.5 s**, i.e. **1.33× slower / 1.41× faster / 2.04× faster** against the same 7.1 s BEDTools reference, with accuracy relabeled from mean |ΔJ| against hammock's own baseline to MAE (1.15e-3 / 1.56e-3 / 1.39e-3) against exact BEDTools. The register-equality numbers in this paragraph are preserved here as the historical record of that harness re-measurement and are what Supplementary Figure S9's register-equality bars still plot (there, now also rescored as MAE vs exact BEDTools, ~0.137–0.138 — see `docs/data/maurano_subB_re_vs_bedtools.csv`).

**Update, 2026-08-10 00:30:** Figure 3 Panel A's small-N cells (N≤32) are now also corrected — job 29671317 (single node, two replicate counts, see the Figure 3 generation note below) replaces job 29656140, whose N=2/N=4 draw was noise-corrupted at n=3. Headline N=512 speedup moved slightly with the new run: 9.2× (using `jaccard_similarity_ie`), not 10.2×. Supplementary Figs S6 (threading, job 29670792), S7 (catalog scale, re-rendered against this same corrected CSV), and S8 (precision frontier, job 29670793) have all been re-measured on the fixed `sweep.py`/`run_bedtools()` path and regenerated. See `docs/bedtools-baseline-retraction.md`'s "Re-measured and gated" section for the current numbers. The "process creation caps near 123 exec/s" mechanism they used to cite is retracted (see above); their captions no longer depend on it.

**Update, 2026-08-10 (figure simplified):** Figure 3 now reports the `jaccard_similarity_ie` column exclusively, in place of register-equality (`jaccard_similarity`), in both panels — the register-equality column carries a chance-agreement floor and is not on BEDTools' scale (CLAUDE.md divergence #2), so it is not a fair thing to plot next to a BEDTools-calibrated speed/accuracy comparison without extensive caveats. Panel A dropped the paired "default"/`+IE` line series down to one hammock curve; the N=512 annotation is now a single number (8.4×, not the earlier two-number 9.2×/8.4× pairing). Panel B dropped its short-lived paired-bar design back to one bar per condition, now reading wall time at the `+IE` arm and MAE of `jaccard_similarity_ie` against exact BEDTools (not mean |ΔJ| against hammock's own baseline, which is what this panel reported before). The full register-equality-vs-`+IE` picture — both panels, both hammock variants, Panel B's register-equality bars now *also* scored as MAE vs exact BEDTools so the two are comparable at all — is preserved as Supplementary Figure S9 (`docs/paper_outline.md` / `paper/draft.md`), for the supplement or advisor review rather than the main text.
...
**Figure 3 generation.** The combined figure is generated by `paper/pairwise_scaling/plot_pairwise_scaling.R` and written to `paper/figures/pairwise_scaling.png`. Panel A uses `docs/data/cpp_vs_bedtools_t16_p18.csv`, job **29671317**, node **c529**, one exclusive allocation, 2026-08-09/10 — two passes within the same allocation, merged: N∈{2,4,8,16,32} at 20 replicates, N∈{64,128,256,512} at 3, both `--metrics-arm`. This replaces job 29656140 (node c531), whose N=2/N=4 cells were noise-corrupted at n=3 — a 20-replicate recheck (job 29670970, node c431) found bedtools winning 20/20 at N=2, the opposite sign from the published n=3 draw, and job 29671317 was designed as a single-node rerun specifically so the corrected small-N cells and the existing large-N cells would not carry a node confound at the splice point (see `docs/bedtools-baseline-retraction.md`). Panel B uses `docs/data/maurano_bedtools.csv` and `docs/data/maurano_subB_ie_summary.csv` (the latter new 2026-08-10: median wall time and MAE-vs-exact-BEDTools of the `+IE` arm at each subB level, 5 replicates, p=18, t=8, from `experiments/subB_mixed_stride/results/sweep_maurano_ie_20260809_200658.csv`). **Do not compare Panel A's absolute times to any earlier run.** The p=14 predecessor (job 29552415, node sr14) was itself a genuine allocated SLURM job; the "1.27–1.53× slower, no SLURM allocation" finding belongs to a *different*, unrelated 2026-08-04 file (`pairwise_cost_by_precision_20260804_164807.csv`) compared against its 08-08 successor, not to job 29552415 (`docs/seed-benchmark-methodology.md` distinguishes the two — conflating them was an error in an earlier draft of this note). Regardless of mechanism, within-run ratios survive cross-run confounds; absolutes do not. The script rebuilds both panels with harmonized typography, panel labels, tool colors, wall-time units, and the pair count N^{2}.
...
- **Cost.**
  ...
  - Measured overhead, same job and binary, 16 threads, **p = 18** (`docs/data/cpp_vs_bedtools_t16_p18.csv`, job 29671317, node c529 — supersedes job 29656140/c531, corrected for a small-N noise bug, see the Figure 3 generation note): the comparison *phase* costs **1.64–1.94×** more for N ≥ 64, and that phase is **1.0% of wall at N = 32 rising to 14.1% at N = 512**, so **total wall rises 10.0% at N = 512** (78.08 s against 70.97 s) — 0.7% at N = 32, 2.6% at N = 128, 5.3% at N = 256. Peak RSS 278 MB against 266 MB.
  - **This bullet previously read "1.45% at N = 512, negligible across the entire measured range" and that was a p = 14 measurement.** The phase multiplier *fell* moving to p = 18 (3.4–3.8× → 1.6–1.9×) but the phase's *share of wall* rose about thirty-fold, because raising precision makes each pairwise comparison more expensive while sketching cost grows far more slowly. The two effects work in opposite directions and the share is the one that reaches the user. Quote the overhead with its N **and** its p, or not at all.
  ...
  - ~~State explicitly which configuration Figure 3 benchmarks, so the speed claim and the reporting recommendation refer to the same thing.~~ **Done, 2026-08-09, then simplified further 2026-08-10**: Figure 3A briefly plotted both total-wall curves (default column and `+IE`) with both N=512 speedups (9.2×/8.4×) in one annotation; as of the same-day figure simplification it plots `jaccard_similarity_ie` only, so the speed claim and the reporting recommendation now refer to literally the same run, not two adjacent ones. The dual-curve view (both columns, both N=512 numbers) moved to Supplementary Figure S9 rather than being deleted.
