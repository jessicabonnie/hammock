...
**Figure 6. Sequence-mode sketches recover fetal-tissue organization in the Maurano DNase hypersensitivity collection.** Twenty fetal-tissue DNase-seq interval sets spanning ten tissue labels were converted to interval-derived sequence and compared using the minimizer-only sequence-mode Jaccard at \(k=10\), \(w=30\), and HyperLogLog precision \(p=12\), the configuration that maximized agreement between the resulting clusters and the tissue annotations. Average-linkage hierarchical clustering was performed on \(1-J\). Leaf labels show dataset accession IDs and are colored by tissue; the legend maps colors to tissue labels. Light neutral outlines mark the ten clusters obtained by cutting the dendrogram at the number of annotated tissue classes. This cut yielded an adjusted Rand index of \(0.910\) and normalized mutual information of \(0.961\). Most tissue groups form distinct clades, including heart, small intestine, and lung. The remaining discrepancies are concentrated in the two brain samples, which separate into different clusters, and among muscle subtypes, where one back-muscle sample groups with the leg-muscle samples. Thus, sequence-mode similarity preserves most of the biological organization of this broader tissue collection while retaining finer structure among related tissue subtypes.

**Figure 6 generation.** The figure is generated by `paper/sequence_tissue_clustering/plot_sequence_tissue_clustering.R` and written to `paper/figures/sequence_tissue_clustering.png`. By default, the script reads the ARI-optimal output `experiments/maurano_dhs_validation/results/raw_d/hammock_mnmzr_p12_jaccD_k10_w30.csv` and the tissue key `experiments/maurano_dhs_validation/data/maurano_filenames_key.tsv`. Optional command-line arguments can override the similarity CSV, tissue key, and output path. All figure colors are routed through `PAPER_PALETTE` and `TISSUE_COLORS`; when `paper/palette.R` is added, the script will automatically use compatible objects defined there.
