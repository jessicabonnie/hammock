...
![Figure S5b](figures/sequence_tissue_clustering_precision.png)

**Figure S5b. Tissue recovery at the biological optimum is stable across HyperLogLog precision despite changes in dendrogram topology.** The same 20 fetal-tissue DNase-seq interval sets were compared at k=10 and w=30 using the inclusion–exclusion Jaccard estimate at **(A)** p=12, **(B)** p=18, and **(C)** p=24. Each panel shows average-linkage agglomerative hierarchical clustering on 1−J; leaf labels are colored and boxed by annotated tissue. Cutting each hierarchy into ten clusters gives the same partition at all three precisions (ARI=0.910 and NMI=0.961), although branch heights, topology, and leaf order differ. Thus the reported tissue-recovery result does not depend on selecting a high sketch precision.

**Figure S5b generation.** Panels A–C are generated with `paper/sequence_tissue_clustering/plot_sequence_tissue_clustering.R` from the p=12, p=18, and p=24 k=10, w=30 Mode D outputs using `jaccard_similarity_ie`. `paper/sequence_tissue_clustering/combine_precision_dendrograms.R` stacks the three panels as rows in `paper/figures/sequence_tissue_clustering_precision.png`.
