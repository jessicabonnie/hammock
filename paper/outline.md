...
**Figure 7. Numerical agreement and biological resolution favor different sequence-mode parameter settings.** Sequence-mode configurations were evaluated on the 20 Maurano fetal-tissue DNase hypersensitivity interval sets at HyperLogLog precision \(p=24\) using the minimizer-only `jaccard_similarity` estimator. Both panels share the same ordered minimizer-window axis and the same color and symbol for each \(k\); a light dashed guide marks \(w=30\). (A) Pearson correlation with exact BEDTools Jaccard increases toward a broad optimum at larger k-mer and minimizer-window sizes: the \(k \ge 15\) curves form a plateau near \(r=1\) spanning \(w=20\) to \(w=300\), whereas \(k=8\) never exceeds \(r=0.83\). The highest correlation, \(r=0.9996\) at \(k=20, w=100\), is marked with an open symbol; the lowest mean absolute error over the same sweep, \(0.0061\), occurs at the adjacent setting \(k=25, w=100\). (B) Agreement between the resulting hierarchical clusters and annotated fetal-tissue labels, measured by the adjusted Rand index, instead reaches a distinct maximum at \(k=10\) and \(w=30\)—the setting used for the Figure 6 dendrogram—and falls away sharply on either side. Every \(k \ge 15\) configuration yields the same adjusted Rand index, \(0.693\), at every window size, so those three curves coincide exactly; series are drawn with a small constant horizontal offset so that coincident points remain visible. The configurations that best reproduce exact interval similarity therefore recover markedly less of the annotated tissue organization than the selected setting (\(\mathrm{ARI}=0.693\) versus \(0.910\)), and numerical agreement alone is not a sufficient criterion for choosing sequence-mode parameters. \(k=5\) is omitted because it was swept at only two window sizes, one of which yields an undefined correlation.

**Figure 7 generation.** The figure is generated by `paper/parameter_response/plot_parameter_response.R` and written to `paper/figures/parameter_response.png`. The single input is `docs/data/mode_d_summary.csv` (staged from `experiments/maurano_dhs_validation/results/mode_d_summary.csv`), filtered to `precision == 24`, `column == "jaccard_similarity"`, and `reference == "bedtools"`. The script computes the annotated best-correlation, lowest-MAE, and Figure 6 settings from that file at run time rather than from fixed constants, and prints all three to stderr. The earlier Pareto-style scatter of Pearson \(r\) against adjusted Rand index (`docs/scripts/mode_d_metric_tradeoff.R`) is retained as a compact supplementary summary of the same sweep.
