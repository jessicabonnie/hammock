...
**Panel A re-measured at p=18, 2026-08-09 (jobs 29652408, 29652432) — resolved.** The earlier BEDTools baseline was superseded: a pairwise `bedtools jaccard` workflow launches one process per pair and was process-creation bound on these nodes (~123 exec/s, not scaling with cores), and the harness ran *three* processes per pair rather than one. It now runs one, records each row's achieved parallel efficiency, and pins BEDTools 2.30.0 (2.27.1's union column is order-dependent — 93 of 190 Maurano pairs gave J(A,B) ≠ J(B,A)). Panel A is redrawn from `docs/data/cpp_vs_bedtools_t16_p18.csv`; the N=512 speedup is **27.61×** at p=18 subB=1.0, against 12.35× in the retired p=14 panel — a precision difference, not an improvement. Two facts to carry into the prose: BEDTools converted only ~1.6 of 16 cores on this corpus (Supplementary Fig S6), so the equal-core-budget framing is stated explicitly and the bedtools-own-optimum framing (16.1×) is given beside it; and **at N=20 on Maurano the two tools are at parity** — 0.98× by medians at t=16, where three BEDTools replicates span 5.89–6.82 s while hammock's span 6.13–6.14 s, so the difference is inside BEDTools' own run-to-run noise. At t=8, where both are stable, hammock is 0.92×, i.e. genuinely slower. That is the expected behaviour below the N≈64 crossover and should be stated rather than smoothed over. Panel B was re-measured on the corrected harness too (job 29652709, node sr06): the BEDTools reference falls 11.08 s → **7.34 s** (the old harness ran three processes per pair), and the three hammock bars become **1.12× slower / 1.64× faster / 2.58× faster** at subB = 1.0 / 0.1 / 0.01, replacing 1.16× / 2.13× / 3.05×. So the subB = 1.0 bar now says hammock is *slower* on this corpus — consistent with the parity result above and with the N ≈ 64 crossover, and the panel label is generated from the sign rather than hardcoded to "faster".
