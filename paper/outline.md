...
**Figure 7. Numerical agreement and biological resolution favor different sequence-mode parameter settings.** Sequence-mode configurations were evaluated on the 20 Maurano fetal-tissue DNase hypersensitivity interval sets at HyperLogLog precision p=24 using the minimizer-only `jaccard_similarity` estimator. Each point represents one (k, w) configuration, with agreement to exact BEDTools Jaccard (Pearson r) on the x-axis and recovery of annotated tissue structure (adjusted Rand index) on the y-axis. k-mer size is encoded by point color and minimizer-window size by point size. A dashed line connects Pareto-optimal configurations, making the trade-off between the two objectives explicit. The configuration with the highest Pearson correlation and the configuration with the highest ARI are highlighted; the k=10, w=30 setting used for Figure 6 is also marked when distinct. The figure therefore shows directly that the sequence-mode parameters that most closely reproduce coordinate-based similarity are not necessarily those that best preserve biological organization.

**Figure 7 generation.** The figure is generated by `paper/parameter_objective_tradeoff/plot_parameter_objective_tradeoff.R` and written to `paper/figures/parameter_objective_tradeoff.png`. The input is `docs/data/mode_d_summary.csv` (staged from `experiments/maurano_dhs_validation/results/mode_d_summary.csv`), filtered to `precision == 24`, `column == "jaccard_similarity"`, and `reference == "bedtools"`; configurations with k<8 are excluded. The script computes the best-Pearson, best-ARI, Figure 6, and Pareto-frontier annotations directly from the input.
