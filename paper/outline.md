...
### Supplementary Figure S1 — interval accuracy at the default precision

![Figure S1](figures/interval_accuracy_p18.png)

**Figure S1. Inclusion–exclusion estimates closely reproduce exact interval Jaccard at the default HyperLogLog precision.** (A) Pairwise comparisons on 20 Maurano et al. fetal-tissue DNase hypersensitivity BED files yielded 190 unique off-diagonal pairs after self-comparisons and reciprocal duplicates were excluded. At the hammock CLI default, p=18, `jaccard_similarity_ie` closely tracked exact BEDTools base-pair Jaccard (MAE =0.00115, Pearson r=0.99991, and Kendall τ=0.9883). (B) The precision–runtime analysis from Figure 4 is repeated for context. Across p=12–24, accuracy is the MAE over the same 190 unique pairs and wall time is the median of three complete all-pairs runs using 16 threads and `subB=1.0`; vertical ranges show the observed replicate range. The circled p=18 point marks the CLI default.

**Figure S1 generation.** `paper/interval_accuracy/plot_interval_accuracy.R` with its optional reference-precision argument set to 18, using `docs/data/maurano_bedtools_ref.tsv`, `docs/data/hammock_hll_p18_jaccB_full.csv`, and `docs/data/sweep_precision_maurano_p18_t16.csv`; output is `paper/figures/interval_accuracy_p18.png`.
