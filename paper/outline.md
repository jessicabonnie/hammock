...
**Panel A and B re-measured on the corrected harness, 2026-08-09 (jobs 29656140, run_bedtools_maurano rerun) — this is the current, checked number.** Four harness defects were found and fixed the same day, all of which had inflated the BEDTools baseline (full history: `docs/bedtools-baseline-retraction.md`). The last and largest: the BEDTools environment module exports `LD_LIBRARY_PATH` with 17 directories of which BEDTools uses 4, and the dynamic linker searched the other unused ones — on a network filesystem — on every one of N² process launches. Fixing it moved the **N=512 speedup from a previously-reported 27.61× to 10.17×** — not a precision change, a bug fix, and the earlier number should not be quoted. Verified: hammock's own wall time at N=512 is unchanged across the fix (71.65 s → 70.32 s), which is the control confirming only the BEDTools arm moved; the corrected CSV passes a monotonicity check at every N except N=2, where three replicates span 0.14–1.22 s (CV 102%, ~15 ms/pair of real work dominated by process-launch jitter at n=3) — a known instability (`docs/seed-benchmark-methodology.md` item 6), not a new defect.

BEDTools' achieved parallelism on this corpus is now 2.3–4.3 of 16 cores across N=8–512 (`mean_bedtools_parallel_eff`) — still far from saturated, so the equal-core-budget framing should be stated explicitly. Panel B (Maurano, 20 files, subB sweep) was re-measured on the same fixed call path: the BEDTools reference is **7.07 s** (was 7.34 s under the partial fix, 11.08 s originally), and the hammock bars read **1.16× slower / 1.58× faster / 2.49× faster** at subB = 1.0 / 0.1 / 0.01. hammock does not beat BEDTools without subsampling on this 20-file corpus — consistent with N=20 sitting well below the crossover visible in Panel A (roughly N≈40–64, where the two curves meet). The panel B label is generated from the sign, not hardcoded to "faster".

**Not yet re-measured with this fix:** Supplementary Figs S6 (threading), S7 (catalog scale), S8 (precision frontier) — all three still call the unfixed `sweep.py` BEDTools path. Treat their BEDTools-relative numbers, and the "process creation caps near 123 exec/s" mechanism they're built on, as retracted until rerun. See `docs/bedtools-baseline-retraction.md`.

![Figure 3](figures/pairwise_scaling.png)

**Figure 3. Hammock expands feasible all-pairs comparison as interval collections grow.** (A) Wall time for hammock and BEDTools across synthetic collections containing 10,000 intervals per BED file, using HyperLogLog precision p=18 (the hammock CLI default), 16 threads, and three runs per configuration; N files per side gives N^{2} ordered pairs, 262,144 at N=512. Timings are of the standalone \texttt{hammock-cpp} binary, which the wheel installs but does not place on \texttt{PATH}. "BEDTools at 16 threads" is 16 concurrent single-threaded \texttt{bedtools jaccard} processes under GNU \texttt{parallel}, since \texttt{bedtools jaccard} has no batch or threaded mode; each BEDTools row records the parallelism it actually achieved, which on this corpus is 1.6 of 16 cores (Supplementary Fig S6). Curves show total hammock wall time as well as sketch-construction and sketch-comparison components; the `+IE` comparison curve includes `jaccard_similarity_ie`, containment, and co-sketch output. (B) Wall time on 20 Maurano fetal-tissue DNase hypersensitivity BED files using interval mode with p=18 and eight threads. Both counts are reported because they describe different things: 400 ordered pairs are *timed*, while the 190 unique off-diagonal pairs carry the reported mean |\Delta J| (self-comparisons excluded). Bars compare BEDTools with hammock at `subB=1.0`, `0.1`, and `0.01`; labels report wall time, speedup relative to BEDTools, and mean |\Delta J| relative to hammock's unsubsampled similarity estimates.
...
**Figure 3 generation.** The combined figure is generated by `paper/pairwise_scaling/plot_pairwise_scaling.R` and written to `paper/figures/pairwise_scaling.png`. Panel A uses `docs/data/cpp_vs_bedtools_t16_p18.csv` (job 29652408, node c595, one exclusive allocation, 2026-08-09), which replaces the p=14 `cpp_vs_bedtools_t16_20260804_172242.csv`; Panel B uses `docs/data/maurano_subB_summary.csv` and `docs/data/maurano_bedtools.csv`. **Do not compare Panel A's absolute times to any earlier run** — the p=14 predecessor had no SLURM allocation and measured 1.27–1.53× slower on byte-identical code (`docs/seed-benchmark-methodology.md`). Within-run ratios survive that; absolutes do not. The script rebuilds both panels with harmonized typography, panel labels, tool colors, wall-time units, and the pair count N^{2}.
...
- **Cost.**
  ...
  - Measured overhead, same job and binary, 16 threads, **p = 18** (`docs/data/cpp_vs_bedtools_t16_p18.csv`, job 29652408): the comparison *phase* costs **1.58–1.86×** more for N ≥ 8, and that phase is **1.0% of wall at N = 32 rising to 15.1% at N = 512**, so **total wall rises 8.5% at N = 512** (77.77 s against 71.65 s) — 0.5% at N = 32, 2.9% at N = 128, 4.9% at N = 256. Peak RSS 278 MB against 266 MB.
