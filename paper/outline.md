...
**Figure 7. Sequence parameters determine biological recovery, while similarity estimator choice primarily changes numerical calibration.** Sequence-mode parameter configurations were evaluated on 20 Maurano fetal-tissue DNase hypersensitivity interval sets at HyperLogLog precision p=24 using the same minimizer-derived HLL sketches but two similarity estimators. Panel A uses Hammock's register-equality statistic (`jaccard_similarity`); Panel B uses the inclusion–exclusion Jaccard estimate (`jaccard_similarity_ie`). In both panels, each point is one combination of k-mer size (k) and minimizer-window size (w), with agreement to exact BEDTools Jaccard across pairwise comparisons (Pearson r) on the x-axis and recovery of the ten annotated tissue classes (adjusted Rand index, ARI) on the y-axis. Point color denotes k and point size denotes w. The estimator-specific numerical optimum is outlined in orange, whereas the k=10, w=30 biological optimum used for Figure 6 is marked by a diamond. The numerical optimum shifts from k=20, w=100 under register equality (r=0.9996) to k=20, w=20 under inclusion–exclusion (r=0.99997), while the biological optimum remains k=10, w=30 with ARI=0.9102 under both estimators. At p=24, ARI differs between the estimators at only 1 of 39 parameter cells; that exception, k=8, w=500, is marked with a grey outline and is degenerate under both estimators (ARI 0.059 versus 0.025). At the Figure 6 setting, the two estimators induce the identical ten-cluster partition, not merely the same ARI. Thus estimator choice substantially changes numerical calibration and the location of the BEDTools-matching optimum, while the biologically useful parameter region and selected tissue-recovery optimum are stable at the precision used in the manuscript.
