...
# Figure 3 (CANDIDATE v2) — Pairwise scaling of hammock versus BEDTools
# Creates a publication-ready two-panel PNG using CairoPNG.
#
# This is a side-by-side candidate for plot_pairwise_scaling.R, not a
# replacement. It writes to paper/figures/pairwise_scaling_v2.png so the
# published figure and this one can be opened together. v1 is left byte-
# identical because it is what generated the currently-referenced PNG.
#
# Three differences from v1:
#
#   1. A fourth Panel A series, "hammock sketch comparison (+IE)", measuring
#      the pairwise phase with jaccard_similarity_ie and the containment/
#      cosketch block emitted -- the configuration CLAUDE.md tells readers to
#      use, and the one no published timing has ever included. It comes from
#      the `hammock_ie_B` arm added to benchmark_cpp_vs_bedtools.py in v0.7.0,
#      measured in the same job as the series beside it.
#   2. The secondary x axis counts N^2, not N(N-1)/2. The harness builds two
#      DISJOINT sets of N files and both tools run the full cross product
#      (benchmark_cpp_vs_bedtools.py builds set1_*/set2_*; hammock_cli.cpp
#      loops n x m; bedtools.sh is `parallel ::: file1 ::: file2`), so N=512 is
#      262,144 pairs, not 130,816. make_graphs.R already had this right.
#      Panel B is untouched -- its "190 unique pairs" IS N(N-1)/2, because that
#      run is a single within-set all-pairs comparison.
#   3. The pmax(comparison_time, 1e-4) floor is gone. It existed because the
#      binary reported integer milliseconds, so the comparison series was a
#      floor constant rather than a measurement at N in {2,4,8,16}. v0.7.0
#      reports microseconds. This changes the shape of the EXISTING comparison
#      series at low N independently of anything else here.
#
# To adopt: replace plot_pairwise_scaling.R with this file, point it back at
# paper/figures/pairwise_scaling.png, and update the Figure 3 caption in
# paper/outline.md (the N(N-1)/2 sentence at :42 and :44).
...
out_png <- if (length(argv) >= 1) {
  ...
  file.path(repo_root, "paper", "figures", "pairwise_scaling_v2.png")
}
