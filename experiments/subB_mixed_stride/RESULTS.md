...
## `--metrics` IE-capture arm retargeted to bare default (2026-08-11)

Part 9 of `docs/seed-metrics-column-restructure.md`: `run_sweep.py`'s own
`--metrics` flag (the second leg of `sbatch_maurano.sh`, and
`sbatch_synthetic_ie.sh`'s only leg) used to pass hammock-cpp's `--metrics`
(the old 8-column full block) to capture `jaccard_similarity_ie`. Since
v0.8.0 the bare default (no flag) gives that column alone, and
`summarize_synthetic_ie.py` — the only live consumer of this arm's output —
was confirmed (re-read 2026-08-11, not just trusted from the seed doc) to
read neither `jaccard_ie` nor `containment_AB`/`containment_BA`, only
`wall_time`/`threads`/`subB`/`method`/`size_class`/`run_id`/`file_a`/
`file_b`. So `run_sweep.py` now invokes the bare default for this arm
(`parse_hammock_csv` was also fixed to read `jaccard_similarity_ie` as the
`jaccard` field's fallback, since the bare shape has no `jaccard_similarity`
column at all and rows would otherwise be silently dropped).

This is the arm behind `docs/data/maurano_subB_ie_summary.csv`'s
`wall_median` column, which is what CLAUDE.md's "1.87×" Figure 3B number
divides (9.404 s at subB=1 → 5.027 s at subB=0.1). Per the C++ code
(`hammock_cli.cpp`), Ie and Full run the *identical* per-pair fused-union
computation — Full just additionally writes cell[1..7] and the extra
`fprintf` columns — so the two arms were only ever expected to differ in
write cost, not compute cost, and at N=20 (190 pairs, 400 cells) that write
cost is a tiny fraction of wall time next to sketch construction.

Measured directly: 5 interleaved reps, arm order permuted per replicate
(mirroring job 29628907's paired design), hammock-cpp 0.8.0, Maurano corpus,
mixed-stride, subB=1.0, p=18, t=8 — matching the exact cell
(`maurano_subB_ie_summary.csv`'s subB=1 row) the cited number comes from:

| arm | wall (s), 5 reps | median |
|---|---|---|
| bare default (new) | 9.432, 9.516, 9.529, 9.408, 9.478 | 9.478 |
| `--metrics` full block (old) | 9.418, 9.430, 9.502, 9.451, 9.535 | 9.451 |

Ratio 0.997–1.003 either direction — **well within the ±2–4% noise floor**
established in `docs/seed-benchmark-methodology.md`. **Retargeted, no
material change.** `docs/data/maurano_subB_ie_summary.csv` and the "1.87×"
number it feeds (CLAUDE.md, `paper/outline.md`/`draft.md`, Figure 3B) are
left as-is — not a hammock speedup, not a regression, just a flag/shape
change measured to be a no-op at this N.
