...
@pytest.mark.parametrize("shape,header", [(s, h) for s, _f, _t, h in _SHAPES])
def test_shape_row_length_matches_header(corpus, tmp_path: Path, shape, header):
    """csv.DictReader silently tolerates a row with the wrong field count
    (extra values go to the None restkey, missing ones get restval) -- a
    header/stride desync in the C++ output (e.g. a stale `cell[]` write left
    over from a column-count change) would not be caught by the
    column-by-column checks elsewhere in this file. Parse raw lines instead,
    mirroring tests/test_metrics_flags.py's `len(row) == len(header)` checks.
    """
    q, r = corpus
    cpp_path = _run_cpp_path(q, r, tmp_path / f"len_{shape}", shape)
    lines = cpp_path.read_text().splitlines()
    assert lines[0].split("\t") == header
    for line in lines[1:]:
        row = line.split("\t")
        assert len(row) == len(header), (shape, row)
