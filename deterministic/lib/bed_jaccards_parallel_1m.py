...
def process_file2(args):
    ...
    for i in range(len(primary_keys)):
        ...
        unionA, unionB, jaccA, jaccB = calculate_jaccard_similarity2(primary_sets[primary_keys[i]], comparator, mode)
        output[primary_keys[i]] = (unionA, unionB, jaccA, jaccB)
    return basename, output
    
    # return basename, bed_to_sets(filename=filepath, mode=mode, comparator=comparator)
def process_file(filepath):
    basename = os.path.basename(filepath)
    return basename, bed_to_sets(filename=filepath, mode=mode)

def calculate_jaccard_similarity2(set1, set2, mode):
    # set1, set2, mode = args
...
if __name__ == "__main__":
    ...
    # [ os.path.basename(f) for f in primepaths]
    prime_keys.sort()
    ...
    with Pool() as pool:
        ...
        args_list = [(f, prime_sets, prime_keys, mode) for f in filepaths]
        print([len(g) for g in args_list])
        print(len(args_list))
        print(type(args_list[0][1]))
        # exit()
        # exit()
        # Map the worker function to the arguments list and get the results
        results = pool.map(process_file2, args_list)
    ...
    # print(prime_keys)
    # secondary_keys = list(set(bed_sets.keys()))
    # secondary_keys.sort()
    ...
    # with Pool() as pool:
    #     # Create a list of arguments for each worker
    #     args_list = [(i, j, bed_keys, bed_sets, mode) for i in range(len(prime_keys)) for j in range(i, len(bed_keys))]
    #     # Map the worker function to the arguments list and get the results
