...
public:
...
#if THREADSAFE
        // Atomically update register to max(current, lzt)
        // Read current value atomically before each CAS attempt to avoid TOCTOU race
        uint8_t current_val = __atomic_load_n(core_.data() + index, __ATOMIC_ACQUIRE);
        while(current_val < lzt) {
            // Try to update: if another thread updated it, CAS fails and we retry
            if(__atomic_compare_exchange_n(core_.data() + index, &current_val, lzt, 
                                          false, __ATOMIC_RELEASE, __ATOMIC_ACQUIRE)) {
                break;  // Successfully updated, done
            }
            // CAS failed: current_val now contains the actual current value, retry if still needed
        }
#else
    ...
    double jaccard_hll_ratio(const hll_t& other) const;
    double jaccard_register_match(const hll_t& other) const;
    // Register-based Jaccard similarity calculation
    double jaccard_similarity_registers(const hll_t& other) const;
    // Dashing-style super-register Jaccard (groups of k registers; averaged across offsets)
    double jaccard_super_registers(const hll_t& other, std::size_t super_size = 4) const;
    // Direct harmonic-weighted Jaccard from registers (no cardinality estimation)
    double jaccard_harmonic_direct(const hll_t& other) const;
    
    // MLE cardinality estimation methods
    double report_mle() noexcept;
    double report_ertl_mle() noexcept;
    double report_ertl_improved() noexcept;
};
